...
public final class NetWorkCenter {
     ...
     * 日志输出组件
     */
    private static final Logger LOG = Logger.getLogger(NetWorkCenter.class);
    ...
    public static BasicResult post(String url, List<NameValuePair> pairs) {
        final BasicResult[] response = new BasicResult[]{null};
        doRequest(RequestMethod.POST, url, pairs, null, new ResponseCallback() {
            @Override
            public void onResponse(int resultCode, String resultJson) {
                response(response, resultCode, resultJson);
            }
        });
        return response[0];
    }

    public static BasicResult post(String url, String paramData) {
        final BasicResult[] response = new BasicResult[]{null};
    ...
    protected static void response(final BasicResult[] response, int resultCode, String resultJson) {
        if (200 == resultCode) {
            BasicResult r = JSON.parseObject(resultJson, BasicResult.class);
            if (StringUtils.isEmpty(r.getErrcode())) {
                r.setErrcode("0");
            }
            r.setErrmsg(resultJson);
            response[0] = r;
        } else {//请求本身就失败了
            response[0] = new BasicResult();
    ...
    public static BasicResult post(String url, String paramData, List<File> fileList) {
        final BasicResult[] response = new BasicResult[]{null};
    ...
    public static BasicResult get(String url, Map<String, String> paramMap) {
        final BasicResult[] response = new BasicResult[]{null};
        get(url, paramMap, new ResponseCallback() {
            @Override
            public void onResponse(int resultCode, String resultJson) {
                response(response, resultCode, resultJson);
            }
    ...
    public static BasicResult get(String url) {
        final BasicResult[] response = new BasicResult[]{null};
    ...
    private static void doRequest(final RequestMethod method, final String url,
                                  final Object paramData, final List<File> fileList, final ResponseCallback callback) {
        //如果url没有传入，则直接返回
        ...
        switch (method) {
                ...
                if (null != fileList && !fileList.isEmpty()) {
                    ...
                    for (File file : fileList) {
                        ...
                        if (file.isFile()) {
                            ...
                            LOG.warn("The target '{" + file.getPath() + "}' not a file,please check and try again!");
                            return;
                        }
                    }
                    if (null != paramData) {
                        builder.addPart("description", new StringBody(JSON.toJSONString(paramData), ContentType.APPLICATION_JSON));
                    }
                    ((HttpPost) request).setEntity(builder.build());
                } else {//不上传文件的普通请求
                    if (null != paramData) {
                        if (paramData instanceof List) {
                            List<NameValuePair> object = (List<NameValuePair>) paramData;
                            try {
                                ((HttpPost) request).setEntity(new UrlEncodedFormEntity(object));
                            } catch (UnsupportedEncodingException e) {
                                e.printStackTrace();
                            }
                        } else {
                            // 目前支持JSON格式的数据
                            StringEntity jsonEntity = new StringEntity(JSON.toJSONString(paramData), ContentType.APPLICATION_JSON);
            ...
            case DELETE:
            default:
                LOG.warn("-----------------请求类型:{" + method.toString() + "} 暂不支持-----------------");
                break;
        }
