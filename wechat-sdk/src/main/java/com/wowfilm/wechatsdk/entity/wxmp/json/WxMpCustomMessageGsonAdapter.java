...
public class WxMpCustomMessageGsonAdapter implements JsonSerializer<WxMpCustomMessage> {
  ...
  public JsonElement serialize(WxMpCustomMessage message, Type typeOfSrc, JsonSerializationContext context) {
    ...
    if (WechatConsts.CUSTOM_MSG_TEXT.equals(message.getMsgType())) {
      JsonObject text = new JsonObject();
      text.addProperty("content", message.getContent());
      messageJson.add("text", text);
    }

    if (WechatConsts.CUSTOM_MSG_IMAGE.equals(message.getMsgType())) {
      JsonObject image = new JsonObject();
      image.addProperty("media_id", message.getMediaId());
      messageJson.add("image", image);
    }

    if (WechatConsts.CUSTOM_MSG_VOICE.equals(message.getMsgType())) {
      JsonObject voice = new JsonObject();
      voice.addProperty("media_id", message.getMediaId());
      messageJson.add("voice", voice);
    }

    if (WechatConsts.CUSTOM_MSG_VIDEO.equals(message.getMsgType())) {
    ...
    if (WechatConsts.CUSTOM_MSG_MUSIC.equals(message.getMsgType())) {
    ...
    if (WechatConsts.CUSTOM_MSG_NEWS.equals(message.getMsgType())) {
